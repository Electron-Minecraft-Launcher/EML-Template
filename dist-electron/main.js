...
const require$$3$1 = require("node:os");
const require$$5 = require("node:stream/promises");
const require$$7 = require("node:http");
const require$$8 = require("node:https");
const require$$5$1 = require("node:child_process");
...
const require$$3__namespace = /* @__PURE__ */ _interopNamespaceDefault(require$$3$1);
...
var microsoft = {};
var microsoftgui = {};
var hasRequiredMicrosoftgui;
function requireMicrosoftgui() {
  if (hasRequiredMicrosoftgui) return microsoftgui;
  hasRequiredMicrosoftgui = 1;
  Object.defineProperty(microsoftgui, "__esModule", { value: true });
  const electron_1 = require$$0;
  class MicrosoftAuthGui {
    constructor(mainWindow2, clientId) {
...
var hasRequiredMicrosoft;
function requireMicrosoft() {
  if (hasRequiredMicrosoft) return microsoft;
  hasRequiredMicrosoft = 1;
  var __importDefault = microsoft && microsoft.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  Object.defineProperty(microsoft, "__esModule", { value: true });
  const microsoftgui_1 = __importDefault(requireMicrosoftgui());
  ...
  class MicrosoftAuth {
    ...
    async auth() {
      ...
      if (userCode == "cancel")
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_CANCELLED, "User cancelled the login");
      ...
      if (!response.ok) {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Error getting OAuth2 token: HTTP ${response.status} - ${errorText}`);
      }
    ...
    async refresh(user) {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Error refreshing OAuth2 token: HTTP ${response.status} - ${errorText}`);
      }
      const res = await response.json();
      try {
        return await this.getAccount(res);
    ...
    async getAccount(authInfo) {
      ...
      if (!xboxLiveRes.ok) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Xbox Live Auth failed: ${xboxLiveRes.statusText}`);
      }
      ...
      if (!xstsRes.ok) {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `XSTS Auth failed (Code: ${errCode}). Check Xbox account settings.`);
      }
      ...
      if (!launchRes.ok) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Minecraft Launcher Login failed: ${launchRes.statusText}`);
      }
      ...
      if (!mcLoginRes.ok) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Minecraft Login failed: ${mcLoginRes.statusText}`);
      }
      ...
      if (!hasGameRes.ok) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Failed to check game ownership: ${hasGameRes.statusText}`);
      }
      const hasGame = await hasGameRes.json();
      if (!hasGame.items.some((i) => i.name == "product_minecraft" || i.name == "game_minecraft")) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Minecraft not owned");
      }
      let profile;
      try {
    ...
    async getProfile(mcLogin) {
      ...
      if (!profileRes.ok) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Error while getting the Minecraft profile: ${profileRes.statusText}`);
      }
      const profile = await profileRes.json();
      if (profile.error)
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `Profile Error: ${profile.errorMessage || "Unknown"}`);
      return {
        uuid: profile.id,
        name: profile.name
      };
...
var azuriom = {};
var hasRequiredAzuriom;
function requireAzuriom() {
  if (hasRequiredAzuriom) return azuriom;
  hasRequiredAzuriom = 1;
  ...
  class AzAuth {
    ...
    async auth(username, password, twoFACode) {
      ...
      if (res.status == "pending" && res.reason == "2fa") {
        throw new errors_1.EMLLibError(errors_1.ErrorType.TWOFA_CODE_REQUIRED, "2FA code required");
      }
      if (res.status == "error") {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `AzAuth authentication failed: ${res.reason}`);
      }
      return {
        name: res.username,
        uuid: res.uuid,
    ...
    async verify(user) {
      ...
      if (res.status == "error") {
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, `AzAuth verify failed: ${res.reason}`);
      }
      return {
        name: res.username,
        uuid: res.uuid,
...
function requireRng() {
  ...
  rng.default = rng$1;
  var _nodeCrypto = _interopRequireDefault(require$$0$1);
  function _interopRequireDefault(e) {
    return e && e.__esModule ? e : { default: e };
  }
...
var hasRequiredCrack;
function requireCrack() {
  if (hasRequiredCrack) return crack;
  hasRequiredCrack = 1;
  Object.defineProperty(crack, "__esModule", { value: true });
  const errors_1 = requireErrors$1();
  const uuid_1 = /* @__PURE__ */ requireDist();
  class CrackAuth {
    /**
    ...
    auth(username) {
      if (/^[a-zA-Z0-9_]+$/gm.test(username) && username.length > 2) {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Invalid username");
      }
    }
  }
...
function requireUtils$1() {
  ...
  const node_fs_1 = __importDefault(fs);
  const node_os_1 = __importDefault(require$$3$1);
  ...
  class Utils {
    ...
    getOS() {
      ...
      if (process.platform === "linux")
        return "lin";
      throw new errors_1.EMLLibError(errors_1.ErrorType.UNKNOWN_OS, "Unknown operating system");
    }
    /**
    ...
    getArch() {
      ...
      if (process.arch.includes("32"))
        return "32";
      throw new errors_1.EMLLibError(errors_1.ErrorType.UNKNOWN_OS, "Unknown architecture");
    }
    /**
    ...
    getOSVersion() {
      return node_os_1.default.release();
    }
    /**
    ...
    getServerFolder(serverId) {
      ...
      return node_path_1.default.join(this.getAppDataFolder(), serverId);
    }
    /**
    ...
    async getFileHash(filePath) {
      try {
        ...
        await (0, promises_1.pipeline)(input, hash);
        return hash.digest("hex");
      } catch (err) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.HASH_ERROR, `Error while getting hash of the file ${filePath}: ${err}`);
      }
    }
    /**
    ...
    getLibraryPath(libName, ...path2) {
      const l = libName.replace(/@([a-z]*)$/, "").split(":");
      return node_path_1.default.join(...path2, `${l[0].replace(/\./g, "/")}/${l[1]}/${l[2]}/`);
    }
    /**
    ...
    isNewer(ref, check) {
      ...
      if (ref.name.split("-").pop() !== check.name.split("-").pop())
        return false;
      const vRef = node_path_1.default.join(ref.path, "/").replaceAll("/", "\\").replace(/\\$/, "").split("\\").slice(0, -1).pop().split(".").map((v) => +v.split("-").shift());
      const vCheck = node_path_1.default.join(check.path, "/").replaceAll("/", "\\").replace(/\\$/, "").split("\\").slice(0, -1).pop().split(".").map((v) => +v.split("-").shift());
  ...
  utils$1.default = new Utils();
  return utils$1;
}
var hasRequiredDownloader;
function requireDownloader() {
  if (hasRequiredDownloader) return downloader;
  hasRequiredDownloader = 1;
  var __importDefault = downloader && downloader.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  Object.defineProperty(downloader, "__esModule", { value: true });
  const node_fs_1 = __importDefault(fs);
  const promises_1 = __importDefault(require$$1$1);
  const node_path_1 = __importDefault(path);
  const node_fetch_1 = __importDefault(require$$3);
  const events_1 = __importDefault(requireEvents());
  const utils_1 = __importDefault(requireUtils$1());
  const errors_1 = requireErrors$1();
  const node_http_1 = __importDefault(require$$7);
  const node_https_1 = __importDefault(require$$8);
  class Downloader extends events_1.default {
    /**
    ...
    constructor(dest) {
      ...
      this.httpAgent = new node_http_1.default.Agent({ keepAlive: true, maxSockets: 32 });
      this.httpsAgent = new node_https_1.default.Agent({ keepAlive: true, maxSockets: 32 });
      this.size = 0;
      this.amount = 0;
      ...
      this.lastTime = 0;
      this.lastSize = 0;
      this.dest = node_path_1.default.join(dest);
    }
    /**
    ...
    async download(files, skipCheck = false) {
      ...
      const workers = Array(this.CONCURRENCY_LIMIT).fill(null).map(async () => {
        while (queue.length > 0) {
          const file = queue.shift();
          if (file)
            await this.downloadFileWithRetry(file);
        }
      });
      try {
     ...
     */
    async getFilesToDownload(files) {
      let filesToDownload = [];
      const promises = files.map(async (file) => {
        const filePath = node_path_1.default.join(this.dest, file.path, file.name);
        if (file.type === "FOLDER") {
          try {
        ...
        try {
          await promises_1.default.access(filePath);
          if (file.sha1 && file.sha1 !== await utils_1.default.getFileHash(filePath)) {
            needsDownload = true;
          }
        } catch {
          needsDownload = true;
    ...
    async downloadFileWithRetry(file, attempt = 0) {
      try {
        ...
        if (attempt < 5) {
          ...
          throw new errors_1.EMLLibError(errors_1.ErrorType.DOWNLOAD_ERROR, `Failed to download ${file.name} after 5 attempts`);
        }
      }
    }
    async downloadFile(file) {
      const dirPath = node_path_1.default.join(this.dest, file.path);
      ...
      if (!res.ok || !res.body) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Error while fetching ${file.name}: ${res.statusText}`);
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        res.body.on("data", (chunk) => {
          ...
          if (diffTime > 500) {
            ...
            this.speed = diffSize / (diffTime / 1e3);
            this.lastTime = now;
    ...
    async chmodJavaFiles(filePath, file) {
      if (process.platform !== "win32" && file.executable) {
        await promises_1.default.chmod(filePath, 493);
      }
    }
  }
...
var hasRequiredBootstraps;
function requireBootstraps() {
  if (hasRequiredBootstraps) return bootstraps;
  hasRequiredBootstraps = 1;
  var __importDefault = bootstraps && bootstraps.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  Object.defineProperty(bootstraps, "__esModule", { value: true });
  const downloader_1 = __importDefault(requireDownloader());
  const utils_1 = __importDefault(requireUtils$1());
  const events_1 = __importDefault(requireEvents());
  const node_path_1 = __importDefault(path);
  const node_child_process_1 = require$$5$1;
  const errors_1 = requireErrors$1();
  class Bootstraps extends events_1.default {
    /**
    ...
    async checkForUpdate(currentVersion) {
      let res = await fetch(`${this.url}/bootstraps`, { method: "GET" }).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Error while fetching Bootstrap from the EML AdminTool: ${err}`);
      });
      if (res.version === currentVersion || res.version == null || res.version == "" || res[`${utils_1.default.getOS()}File`] == null)
        return null;
      else
        return res;
    }
    ...
    async download(bootstraps2) {
      const os = utils_1.default.getOS();
      const bootstrap = bootstraps2[`${os}File`];
      if (!bootstrap) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FILE_ERROR, "Not available for this operating system");
      }
      const downloadPath = node_path_1.default.join(utils_1.default.getTempFolder(), bootstrap.path, bootstrap.name);
      const downloader2 = new downloader_1.default(utils_1.default.getTempFolder());
    ...
    runUpdate(bootstrapPath) {
      ...
      const run = (0, node_child_process_1.spawnSync)(cmd);
      if (run.error)
        throw new errors_1.EMLLibError(errors_1.ErrorType.EXEC_ERROR, `Error while executing the Bootstrap: ${run.error}`);
      process.exit();
    }
    /**
    ...
    async checkDownloadAndRun(currentVersion) {
      ...
      if (bootstraps2) {
        try {
          bootstrapPath = await this.download(bootstraps2);
        } catch (err) {
          return;
        }
...
var maintenance = {};
var hasRequiredMaintenance;
function requireMaintenance() {
  if (hasRequiredMaintenance) return maintenance;
  hasRequiredMaintenance = 1;
  ...
  class Maintenance {
    ...
    async getMaintenance() {
      let res = await fetch(`${this.url}/maintenance`, { method: "GET" }).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Error while fetching Maintenance from the EML AdminTool: ${err}`);
      });
  ...
  class News {
    ...
    async getNews() {
      let res = await fetch(`${this.url}/news`, { method: "GET" }).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Error while fetching News from the EML AdminTool: ${err}`);
      });
      return res.news;
    }
    /**
    ...
    async getCategories() {
      let res = await fetch(`${this.url}/news/categories`, { method: "GET" }).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Error while fetching News Categories from the EML AdminTool: ${err}`);
      });
      return res.data;
    }
    /**
...
function requireBackground() {
  ...
  class Background {
     ...
     */
    async getBackground() {
      const res = await fetch(`${this.url}/background`).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Error while fetching backgrounds: ${err}`);
      });
...
function requireServerstatus() {
  ...
  var __createBinding = serverstatus && serverstatus.__createBinding || (Object.create ? (function(o, m, k, k2) {
    if (k2 === void 0) k2 = k;
  ...
  var __setModuleDefault = serverstatus && serverstatus.__setModuleDefault || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
  }) : function(o, v) {
    o["default"] = v;
  });
  var __importStar = serverstatus && serverstatus.__importStar || /* @__PURE__ */ (function() {
  ...
  var __importDefault = serverstatus && serverstatus.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  Object.defineProperty(serverstatus, "__esModule", { value: true });
  const net = __importStar(require$$0$4);
  const errors_1 = requireErrors$1();
  const bufferwriter_1 = __importDefault(requireBufferwriter());
  const bufferreader_1 = __importDefault(requireBufferreader());
  class ServerStatus {
    /**
     ...
     */
    constructor(ip, port = 25565, protocol = "modern", pvn = -1, timeout = 5) {
      this.ip = ip;
      this.port = port;
    ...
    async getStatus() {
      return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          reject(new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, "Connection timed out"));
        }, this.timeout * 1e3);
        socket.on("connect", () => {
          if (this.protocol === "modern") {
            ...
            reject(new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, "Unsupported protocol version"));
          }
        });
          ...
          if (this.protocol === "modern") {
            ...
            if (bufReader.readVarInt() === 0) {
              try {
                ...
                reject(new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, `Received invalid response: ${err}`));
                socket.destroy();
                clearTimeout(timeout);
              }
            } else {
              reject(new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, `Received unexpected packet`));
              socket.destroy();
              clearTimeout(timeout);
            }
          } else {
            if (incomingBuf.readUInt8(0) === 255) {
              ...
              if (fields[0] !== "§1") {
                reject(new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, `Received invalid response: the first field is not '§1'`));
                socket.destroy();
                clearTimeout(timeout);
              }
              resolve({
              ...
              reject(new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, `Received invalid response: wrong packet identifier`));
              socket.destroy();
              clearTimeout(timeout);
            }
          }
        });
        socket.on("error", (err) => reject(new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, err)));
      });
    }
  }
...
var hasRequiredManifests;
function requireManifests() {
  if (hasRequiredManifests) return manifests;
  hasRequiredManifests = 1;
  ...
  class Manifests {
    ...
    async getLoaderInfo(minecraftVersion, url) {
      if (!minecraftVersion && !url)
        return { type: "VANILLA", minecraftVersion: "latest_release", loaderVersion: "latest_release" };
      if (minecraftVersion)
        return { type: "VANILLA", minecraftVersion, loaderVersion: minecraftVersion };
      const res = await fetch(`${url}/api/loader`).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Failed to fetch loader info: ${err.message}`);
      });
      return res;
    }
    /**
    ...
    async getMinecraftManifest(minecraftVersion = "latest_release", url) {
      ...
      const res = await fetch(manifestUrl).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Failed to fetch Minecraft manifest: ${err.message}`);
      });
      return res;
    }
    /**
    ...
    async getMinecraftManifestUrl(minecraftVersion = null) {
      const res = await fetch(consts_1.MINECRAFT_MANIFEST_URL).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Failed to fetch Minecraft version manifest: ${err.message}`);
      });
      ...
      if (!res.versions.find((version2) => version2.id === minecraftVersion)) {
        throw new errors_1.EMLLibError(errors_1.ErrorType.MINECRAFT_ERROR, `Minecraft version ${minecraftVersion} not found in manifest`);
      }
      return res.versions.find((version2) => version2.id === minecraftVersion).url;
    }
    async getJavaManifest(javaVersion, jreV) {
      const url = await this.getJavaManifestUrl(javaVersion, jreV);
      const res = await fetch(url).then((res2) => res2.json()).catch((err) => {
        throw new errors_1.EMLLibError(errors_1.ErrorType.FETCH_ERROR, `Failed to fetch Java manifest: ${err.message}`);
      });
      return res;
    }
    /**
     ...
     */
    async getJavaManifestUrl(javaVersion, jreV) {
      const archMapping = {
      ...
      if (platform !== "win32" && platform !== "darwin" && platform !== "linux") {
        throw new errors_1.EMLLibError(errors_1.ErrorType.UNKNOWN_OS, `Unsupported platform: ${platform}`);
      }
      if (platform === "win32" && arch !== "x64" && arch !== "ia32" && arch !== "arm64" || platform === "darwin" && arch !== "x64" && arch !== "arm64" || platform === "linux" && arch !== "x64" && arch !== "ia32") {
        throw new errors_1.EMLLibError(errors_1.ErrorType.UNKNOWN_OS, `Unsupported architecture: ${arch}`);
      }
      ...
      const fallbackJavaVersion = Object.keys(res[archMapping[platform][arch]]).find((version2) => res[archMapping[platform][arch]][version2][0]?.version.name.split(".")[0] === jreV);
      if (fallbackJavaVersion) {
        return res[archMapping[platform][arch]][fallbackJavaVersion][0].manifest.url;
      }
      throw new errors_1.EMLLibError(errors_1.ErrorType.JAVA_ERROR, `Java version ${javaVersion} not found in manifest`);
    }
  }
...
function requireJava() {
  ...
  hasRequiredJava = 1;
  var __importDefault = java && java.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  Object.defineProperty(java, "__esModule", { value: true });
  const events_1 = __importDefault(requireEvents());
  const manifests_1 = __importDefault(requireManifests());
  const node_path_1 = __importDefault(path);
  const downloader_1 = __importDefault(requireDownloader());
  const utils_1 = __importDefault(requireUtils$1());
  const node_child_process_1 = require$$5$1;
  const errors_1 = requireErrors$1();
  class Java extends events_1.default {
    /**
    ...
    async getFiles(manifest) {
      manifest = manifest ?? await manifests_1.default.getMinecraftManifest(this.minecraftVersion, this.url);
      const jreVersion = manifest.javaVersion?.component ?? "jre-legacy";
      const jreV = manifest.javaVersion?.majorVersion.toString() ?? "8";
      const jreManifest = await manifests_1.default.getJavaManifest(jreVersion, jreV);
      let files = [];
      Object.entries(jreManifest.files).forEach((file) => {
        ...
        if (file[1].type === "directory") {
          files.push({
            name: node_path_1.default.basename(file[0]),
            path: normalizedPath,
            url: "",
            type: "FOLDER"
          });
    ...
    async download() {
      ...
      const downloader2 = new downloader_1.default(utils_1.default.getServerFolder(this.serverId));
      downloader2.forwardEvents(this);
     ...
     */
    async check(absolutePath = node_path_1.default.join(utils_1.default.getServerFolder(this.serverId), "runtime", "jre-${X}", "bin", "java"), majorVersion = 8) {
      return new Promise((resolve, reject) => {
        const javaExec = absolutePath.replace("${X}", majorVersion + "");
        const process2 = (0, node_child_process_1.spawn)(javaExec, ["-version"]);
        let output = "";
        ...
        process2.on("error", (err) => {
          reject(new errors_1.EMLLibError(errors_1.ErrorType.JAVA_ERROR, `Java is not correctly installed: ${err.message}`));
        });
        process2.on("close", (code) => {
          if (code !== 0 && output.length === 0) {
            reject(new errors_1.EMLLibError(errors_1.ErrorType.JAVA_ERROR, `Java exited with code ${code}`));
            return;
          }
          const versionMatch = output.match(/"(.*?)"/);
          const version2 = versionMatch ? versionMatch.pop() : majorVersion + "";
          const arch = output.includes("64-Bit") ? "64-bit" : "32-bit";
          const res = { version: version2, arch };
          this.emit("java_info", res);
          resolve(res);
        });
      });
    }
    normalizeJavaPath(filePath, jreV) {
      ...
      if (filePath.includes(".bundle/")) {
        ...
        return node_path_1.default.join("runtime", `jre-${jreV}`, node_path_1.default.dirname(relativePath), "/");
      }
      return node_path_1.default.join("runtime", `jre-${jreV}`, node_path_1.default.dirname(filePath), "/");
    }
  }
  java.default = Java;
  return java;
...
function requireZipEntry() {
  ...
  var Utils = requireUtil(), Headers2 = requireHeaders(), Constants = Utils.Constants, Methods = requireMethods();
...
var hasRequiredFilesmanager;
function requireFilesmanager() {
  if (hasRequiredFilesmanager) return filesmanager;
  hasRequiredFilesmanager = 1;
  var __importDefault = filesmanager && filesmanager.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  const promises_1 = __importDefault(require$$1$1);
  const node_fs_1 = fs;
  const adm_zip_1 = __importDefault(requireAdmZip());
  const events_1 = __importDefault(requireEvents());
  const java_1 = __importDefault(requireJava());
  class FilesManager extends events_1.default {
    constructor(config, manifest, loader) {
      super();
      this.config = config;
    ...
    async getJava() {
      if (this.config.java.install === "auto") {
        const java2 = await new java_1.default(this.manifest.id, this.config.serverId).getFiles(this.manifest);
    ...
    async getModpack() {
      ...
      const modpack = await fetch(`${this.config.url}/api/files-updater`).then((res) => res.json()).then((res) => res.files).catch((err) => {
    ...
    async getLibraries() {
      let files = [];
      let libraries = [];
      if (!(0, node_fs_1.existsSync)(node_path_1.default.join(this.config.root, "versions", this.manifest.id))) {
        await promises_1.default.mkdir(node_path_1.default.join(this.config.root, "versions", this.manifest.id), { recursive: true });
      }
      files.push({ name: `${this.manifest.id}.json`, path: node_path_1.default.join("versions", this.manifest.id, "/"), url: "", type: "OTHER" });
      await promises_1.default.writeFile(node_path_1.default.join(this.config.root, "versions", this.manifest.id, `${this.manifest.id}.json`), JSON.stringify(this.manifest, null, 2));
      this.manifest.libraries.forEach((lib2) => {
        ...
        if (lib2.natives) {
          ...
          const native2 = lib2.natives[utils_1.default.getOS_MCCode()];
          if (!native2)
            return;
          artifact = classifiers ? classifiers[native2.replace("${arch}", utils_1.default.getArch())] : void 0;
        } else {
          if (!utils_1.default.isLibAllowed(lib2))
            return;
          type = "LIBRARY";
        ...
        if (artifact) {
          if (artifact.path) {
            name = node_path_1.default.basename(artifact.path);
            path2 = node_path_1.default.join("libraries", node_path_1.default.dirname(artifact.path), "/");
          } else {
            name = utils_1.default.getLibraryName(lib2.name);
            path2 = utils_1.default.getLibraryPath(lib2.name, "libraries");
          }
          libraries.push({
            name,
            path: path2,
      ...
      libraries.push({
        ...
        path: node_path_1.default.join("versions", this.manifest.id, "/"),
    ...
    async getAssets() {
      ...
      if (!(0, node_fs_1.existsSync)(node_path_1.default.join(this.config.root, "assets", "indexes"))) {
        await promises_1.default.mkdir(node_path_1.default.join(this.config.root, "assets", "indexes"), { recursive: true });
      }
      files.push({ name: `${this.manifest.assets}.json`, path: node_path_1.default.join("assets", "indexes", "/"), url: "", type: "OTHER" });
      await promises_1.default.writeFile(node_path_1.default.join(this.config.root, "assets", "indexes", `${this.manifest.assets}.json`), JSON.stringify(res, null, 2));
      Object.values(res.objects).forEach((asset) => {
        assets.push({
          name: asset.hash,
          path: node_path_1.default.join("assets", "objects", asset.hash.substring(0, 2), "/"),
     ...
     */
    async extractNatives(libraries) {
      const natives = libraries.filter((lib2) => lib2.type === "NATIVE");
      const nativesFolder = node_path_1.default.resolve(this.config.root, "bin", "natives");
      let files = [];
      if (!(0, node_fs_1.existsSync)(nativesFolder)) {
        await promises_1.default.mkdir(nativesFolder, { recursive: true });
      }
      const promises = natives.map(async (native2) => {
        if (!(0, node_fs_1.existsSync)(node_path_1.default.join(this.config.root, native2.path, native2.name)))
          return;
        const zip = new adm_zip_1.default(node_path_1.default.join(this.config.root, native2.path, native2.name));
        const promisesInner = zip.getEntries().map(async (entry) => {
          if (!entry.entryName.startsWith("META-INF")) {
            ...
            if (!entryName || entryName.includes("..") || node_path_1.default.isAbsolute(entryName)) {
              console.warn(`[Security] Skipped unsafe native extraction: ${entry.entryName}`);
              return;
            }
            const entryPath = node_path_1.default.resolve(nativesFolder, entryName);
            const relative = node_path_1.default.relative(nativesFolder, entryPath);
            const isSafe = relative && !relative.startsWith("..") && !node_path_1.default.isAbsolute(relative);
            if (!isSafe) {
              console.warn(`[Security] Skipped unsafe native extraction: ${entry.entryName}`);
              return;
            }
            if (entry.isDirectory && !(0, node_fs_1.existsSync)(entryPath)) {
              await promises_1.default.mkdir(entryPath, { recursive: true });
            } else {
              const parentDir = node_path_1.default.dirname(entryPath);
              if (!(0, node_fs_1.existsSync)(parentDir))
                await promises_1.default.mkdir(parentDir, { recursive: true });
              const data = zip.readFile(entry);
              if (data)
                await promises_1.default.writeFile(entryPath, data);
            }
            files.push({
              name: node_path_1.default.basename(entryName),
              path: node_path_1.default.join("bin", "natives", node_path_1.default.dirname(entryName), "/"),
              url: "",
              sha1: "",
    ...
    async copyAssets() {
      ...
      if (this.manifest.assets === "legacy" || this.manifest.assets === "pre-1.6") {
        if ((0, node_fs_1.existsSync)(node_path_1.default.join(this.config.root, "assets", "legacy"))) {
        ...
        const assetsContent = await promises_1.default.readFile(node_path_1.default.join(this.config.root, "assets", "indexes", `${this.manifest.assets}.json`), "utf-8");
        const assets = JSON.parse(assetsContent);
        const promises = Object.entries(assets.objects).map(async ([path2, { hash, size }]) => {
          const assetLegacyPath = node_path_1.default.join("resources", node_path_1.default.dirname(path2));
          const assetLegacyName = node_path_1.default.basename(path2);
          if (!(0, node_fs_1.existsSync)(node_path_1.default.join(this.config.root, assetLegacyPath))) {
            await promises_1.default.mkdir(node_path_1.default.join(this.config.root, assetLegacyPath), { recursive: true });
          }
          if (!(0, node_fs_1.existsSync)(node_path_1.default.join(assetLegacyPath, assetLegacyName))) {
            await promises_1.default.copyFile(node_path_1.default.join(this.config.root, "assets", "objects", hash.substring(0, 2), hash), node_path_1.default.join(this.config.root, assetLegacyPath, assetLegacyName));
          }
          files.push({
          ...
          this.emit("copy_progress", { filename: hash, dest: node_path_1.default.join(assetLegacyPath, assetLegacyName) });
        });
        await Promise.all(promises);
      }
...
function requireCleaner() {
  ...
  var __importDefault = cleaner && cleaner.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  const promises_1 = __importDefault(require$$1$1);
  const node_fs_1 = fs;
  class Cleaner extends events_1.default {
    /**
     * @param dest Destination folder.
     */
    constructor(dest) {
     ...
     */
    async clean(files, ignore = [], skipClean = false) {
      if (skipClean)
        return;
      const validFilesSet = new Set(files.map((f) => node_path_1.default.normalize(node_path_1.default.join(this.dest, f.path, f.name))));
      const ignoredPaths = ignore.map((ig) => node_path_1.default.normalize(node_path_1.default.join(this.dest, ig)));
      const deletePromises = [];
      let i = 0;
      this.browsed = [];
      await this.browse(this.dest);
      for (const file of this.browsed) {
        const fullPath = node_path_1.default.normalize(node_path_1.default.join(file.path, file.name));
        ...
        if (!isFileValid && !isIgnored) {
          deletePromises.push(promises_1.default.unlink(fullPath).then(() => {
            i++;
...
var loadermanager = {};
var forgeloader = {};
var hasRequiredForgeloader;
function requireForgeloader() {
  if (hasRequiredForgeloader) return forgeloader;
  hasRequiredForgeloader = 1;
  var __importDefault = forgeloader && forgeloader.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  const utils_1 = __importDefault(requireUtils$1());
  const events_1 = __importDefault(requireEvents());
  class ForgeLoader extends events_1.default {
    constructor(config, manifest, loader) {
      super();
      this.config = config;
      this.manifest = manifest;
     ...
     */
    async setup() {
      const forgePath = node_path_1.default.join(this.config.root, this.loader.file.path);
      const minecraftPath = node_path_1.default.join(this.config.root, "versions", this.manifest.id);
      const zip = new adm_zip_1.default(node_path_1.default.join(forgePath, this.loader.file.name));
      const jar = new adm_zip_1.default(node_path_1.default.join(minecraftPath, `${this.manifest.id}.jar`));
      if (!(0, node_fs_1.existsSync)(forgePath)) {
    ...
    async extractZip(forgePath, minecraftPath, zip, jar) {
      let files = [];
      let i = 0;
      ...
      zip.getEntries().forEach((entry) => {
        ...
        i++;
        this.emit("extract_progress", { filename: node_path_1.default.basename(entry.entryName) });
      });
      await new Promise((resolve, reject) => {
        jar.writeZip(node_path_1.default.join(minecraftPath, `${this.manifest.id}.jar`), (err) => {
          if (err)
            return reject(err);
          resolve();
        });
      ...
      await promises_1.default.writeFile(node_path_1.default.join(forgePath, `${forgeManifest.id}.json`), JSON.stringify(forgeManifest, null, 2));
    ...
    async extractJar(forgePath, zip) {
      ...
      if (installProfile.install) {
        ...
        const jsonEntry = zip.getEntry(node_path_1.default.basename(installProfile.json));
        forgeManifest = JSON.parse(jsonEntry?.getData().toString("utf8") + "");
      }
      await promises_1.default.writeFile(node_path_1.default.join(forgePath, `forge-${this.loader.loaderVersion}.json`), JSON.stringify(forgeManifest, null, 2));
      ...
      if (installProfile.filePath) {
        const universalName = utils_1.default.getLibraryName(installProfile.path);
        const universalPath = utils_1.default.getLibraryPath(installProfile.path);
        const universalExtractPath = node_path_1.default.join(this.config.root, "libraries", universalPath);
        if (!(0, node_fs_1.existsSync)(universalExtractPath))
          await promises_1.default.mkdir(universalExtractPath, { recursive: true });
        const zipEntry2 = zip.getEntry(installProfile.filePath);
        await promises_1.default.writeFile(node_path_1.default.join(universalExtractPath, universalName), zipEntry2.getData());
        libraries.push({ name: universalName, path: node_path_1.default.join("libraries", universalPath), url: "", type: "LIBRARY", extra: "INSTALL" });
        i++;
        ...
        if (!(0, node_fs_1.existsSync)(universalExtractPath))
          await promises_1.default.mkdir(universalExtractPath, { recursive: true });
        const entriesToExtract = zip.getEntries().filter((entry) => node_path_1.default.join(entry.entryName).includes(node_path_1.default.join("maven", universalPath)) && entry.entryName.endsWith(".jar"));
        const promises = entriesToExtract.map(async (entry) => {
          if (!entry.entryName.endsWith(".jar"))
            return;
          await promises_1.default.writeFile(node_path_1.default.join(universalExtractPath, node_path_1.default.basename(entry.entryName)), entry.getData());
          libraries.push({
            name: node_path_1.default.basename(entry.entryName),
            path: node_path_1.default.join("libraries", universalPath),
            url: "",
            type: "LIBRARY",
            extra: "INSTALL"
          });
      ...
      if (installProfile.processors && installProfile.processors.length > 0) {
        ...
        const clientDataName = utils_1.default.getLibraryName(installProfile.path ?? universalMaven.name).replace(".jar", "-clientdata.lzma");
        const clientDataPath = utils_1.default.getLibraryPath(installProfile.path ?? universalMaven.name);
        const clientDataExtractPath = node_path_1.default.join(this.config.root, "libraries", clientDataPath);
        const clientData = zip.getEntry("data/client.lzma").getData();
        if (!(0, node_fs_1.existsSync)(clientDataExtractPath))
          await promises_1.default.mkdir(clientDataExtractPath, { recursive: true });
        await promises_1.default.writeFile(node_path_1.default.join(clientDataExtractPath, clientDataName), clientData);
        files.push({ name: clientDataName, path: node_path_1.default.join("libraries", clientDataPath), url: "", type: "LIBRARY" });
        i++;
    ...
    async getMirrorUrl(lib2) {
      ...
      for (const mirror of mirrors) {
        const url = `${mirror}${utils_1.default.getLibraryPath(lib2.name).replaceAll("\\", "/")}${utils_1.default.getLibraryName(lib2.name)}`;
        try {
          ...
          if (!sha1Res.ok)
            continue;
          const sha12 = await sha1Res.text();
          return { url, size, sha1: sha12 };
        } catch {
          continue;
        }
      }
    ...
    async formatLibraries(libs, extra, installProfile) {
      const promises = libs.map(async (lib2) => {
        let type = "LIBRARY";
        let native2;
        if (lib2.natives) {
          native2 = lib2.natives[utils_1.default.getOS_MCCode()];
          if (!native2)
            return null;
          type = "NATIVE";
        } else {
          if (!utils_1.default.isLibAllowed(lib2) || !lib2.serverreq && !lib2.clientreq && !lib2.url && !lib2.downloads)
            return null;
        }
        let artifact = lib2.downloads?.artifact;
        let name = "";
        let path2 = "";
        let url = "";
        let sha12 = "";
        ...
        if (artifact) {
          if (artifact.path) {
            ...
            path2 = node_path_1.default.join("libraries", artifact.path.split("/").slice(0, -1).join("/"), "/");
          } else {
            name = utils_1.default.getLibraryName(lib2.name);
            if (type === "NATIVE")
          ...
          const mirror = await this.getMirrorUrl(lib2);
          name = utils_1.default.getLibraryName(lib2.name);
          if (type === "NATIVE")
            name = name.replace(".jar", `-${native2}.jar`);
          path2 = utils_1.default.getLibraryPath(lib2.name, "libraries");
      ...
      return results.filter((lib2) => lib2 !== null);
    }
  }
...
function requirePatcher() {
  ...
  var __importDefault = patcher && patcher.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  const node_child_process_1 = require$$5$1;
  const events_1 = __importDefault(requireEvents());
  class Patcher extends events_1.default {
    constructor(config, manifest, loader, installProfile) {
      super();
      this.config = config;
      this.manifest = manifest;
      this.loader = loader;
    ...
    async patch() {
      ...
      for (const processor of processors) {
        if (processor?.sides && !processor.sides.includes("client"))
          continue;
        const jarExtractPathName = node_path_1.default.join(this.config.root, "libraries", utils_1.default.getLibraryPath(processor.jar), utils_1.default.getLibraryName(processor.jar));
        const args = processor.args.map((arg) => this.mapPath(this.mapArg(arg)));
        const classpath = processor.classpath.map((cp) => `"${node_path_1.default.join(this.config.root, "libraries", utils_1.default.getLibraryPath(cp), utils_1.default.getLibraryName(cp))}"`);
        const mainClass = this.getJarMain(jarExtractPathName);
        await new Promise((resolve) => {
          const patch = (0, node_child_process_1.spawn)(`"${this.config.java.absolutePath.replace("${X}", this.manifest.javaVersion?.majorVersion + "" || "8")}"`, ["-classpath", [`"${jarExtractPathName}"`, ...classpath].join(node_path_1.default.delimiter), mainClass, ...args], { shell: true });
          ...
          patch.on("close", (code) => {
            this.emit("patch_progress", { filename: utils_1.default.getLibraryName(processor.jar) });
            i++;
            resolve(code);
          });
        });
    ...
    async isPatched() {
      ...
      const promises = libraries.map(async (lib2) => {
        const libName = utils_1.default.getLibraryName(lib2.replace("[", "").replace("]", ""));
        const libPath = utils_1.default.getLibraryPath(lib2.replace("[", "").replace("]", ""));
        const libExtractPath = node_path_1.default.join(this.config.root, "libraries", libPath);
        const fileObj = { name: libName, path: node_path_1.default.join("libraries", libPath), url: "", type: "LIBRARY" };
        try {
          await promises_1.default.access(node_path_1.default.join(libExtractPath, libName));
    ...
    getJarMain(jarPath) {
      if (!(0, node_fs_1.existsSync)(jarPath))
        return null;
      const manifest = new adm_zip_1.default(jarPath).getEntry("META-INF/MANIFEST.MF")?.getData();
      if (!manifest)
        return null;
    ...
    mapArg(arg) {
      ...
      if (this.installProfile.data[argType]) {
        if (argType === "BINPATCH") {
          const clientDataName = utils_1.default.getLibraryName(this.installProfile.path || universalMaven.name).replace(".jar", "-clientdata.lzma");
          const clientDataExtractPath = utils_1.default.getLibraryPath(this.installProfile.path || universalMaven.name, this.config.root, "libraries");
          return `"${node_path_1.default.join(clientDataExtractPath, clientDataName).replace(".jar", "-clientdata.lzma")}"`;
        }
        return this.installProfile.data[argType].client;
      }
      return arg.replace("{SIDE}", `client`).replace("{ROOT}", `"${this.config.root}}"`).replace("{MINECRAFT_JAR}", `"${node_path_1.default.join(this.config.root, "versions", this.manifest.id, `${this.manifest.id}.jar`)}"`).replace("{MINECRAFT_VERSION}", `"${node_path_1.default.join(this.config.root, "versions", this.manifest.id, `${this.manifest.id}.json`)}"`).replace("{INSTALLER}", `"${node_path_1.default.join(this.config.root, "libraries")}"`).replace("{LIBRARY_DIR}", `"${node_path_1.default.join(this.config.root, "libraries")}"`);
    }
    mapPath(arg) {
      if (arg.startsWith("[")) {
        return `"${node_path_1.default.join(this.config.root, "libraries", utils_1.default.getLibraryPath(arg.replace("[", "").replace("]", "")), utils_1.default.getLibraryName(arg.replace("[", "").replace("]", "")))}"`;
      }
      return arg;
    }
  }
...
var hasRequiredLoadermanager;
function requireLoadermanager() {
  if (hasRequiredLoadermanager) return loadermanager;
  hasRequiredLoadermanager = 1;
  var __importDefault = loadermanager && loadermanager.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  Object.defineProperty(loadermanager, "__esModule", { value: true });
  const events_1 = __importDefault(requireEvents());
  const forgeloader_1 = __importDefault(requireForgeloader());
  const patcher_1 = __importDefault(requirePatcher());
  class LoaderManager extends events_1.default {
    constructor(config, manifest, loader) {
      super();
      this.config = config;
      this.manifest = manifest;
...
var hasRequiredArgumentsmanager;
function requireArgumentsmanager() {
  if (hasRequiredArgumentsmanager) return argumentsmanager;
  hasRequiredArgumentsmanager = 1;
  var __importDefault = argumentsmanager && argumentsmanager.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  class ArgumentsManager {
    ...
    getJvmArgs(libraries) {
      const nativeDirectory = node_path_1.default.join(this.config.root, "bin", "natives").replaceAll("\\", "/");
      const libraryDirectory = node_path_1.default.join(this.config.root, "libraries").replaceAll("\\", "/");
      ...
      if (this.manifest.arguments?.jvm) {
        [...this.manifest.arguments.jvm, ...this.loaderManifest?.arguments.jvm || []].forEach((arg) => {
          if (typeof arg === "string") {
            args.push(arg);
          } else if (arg.rules && utils_1.default.isArgAllowed(arg)) {
      ...
      return args.map((arg) => arg.replaceAll("${natives_directory}", nativeDirectory).replaceAll("${library_directory}", libraryDirectory).replaceAll("${launcher_name}", `${this.config.serverId}-launcher`).replaceAll("${launcher_version}", "2").replaceAll("${version_name}", this.manifest.id).replaceAll("${jar_path}", node_path_1.default.join(this.config.root, "versions", this.manifest.id, `${this.manifest.id}.jar`).replaceAll("\\", "/")).replaceAll("${classpath}", classpath).replaceAll("${max_memory}", this.config.memory.max + "").replaceAll("${min_memory}", this.config.memory.min + "").replaceAll("${classpath_separator}", node_path_1.default.delimiter));
    }
    /**
    ...
    getMinecraftArgs() {
      const gameDirectory = node_path_1.default.join(this.config.root).replaceAll("\\", "/");
      const assetsDirectory = this.manifest.assets === "legacy" || this.manifest.assets === "pre-1.6" ? node_path_1.default.join(this.config.root, "ressources").replaceAll("\\", "/") : node_path_1.default.join(this.config.root, "assets").replaceAll("\\", "/");
    ...
    getClasspath(libraries) {
      ...
      for (let i = 0; i < libraries.length; i++) {
        ...
        if (lib2.type === "LIBRARY") {
          const path2 = node_path_1.default.join(this.config.root, lib2.path, lib2.name).replaceAll("\\", "/");
          ...
          if (check && utils_1.default.isNewer(lib2, check))
            continue;
          classpath.push(path2);
        }
      }
      return [.../* @__PURE__ */ new Set([...classpath])].join(node_path_1.default.delimiter);
    }
    getMainClass() {
...
function requireLauncher() {
  ...
  var __importDefault = launcher && launcher.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  const utils_1 = __importDefault(requireUtils$1());
  const node_path_1 = __importDefault(path);
  const filesmanager_1 = __importDefault(requireFilesmanager());
  const downloader_1 = __importDefault(requireDownloader());
  const cleaner_1 = __importDefault(requireCleaner());
  const java_1 = __importDefault(requireJava());
  const loadermanager_1 = __importDefault(requireLoadermanager());
  const argumentsmanager_1 = __importDefault(requireArgumentsmanager());
  const node_child_process_1 = require$$5$1;
  class Launcher extends events_1.default {
    ...
    constructor(config) {
      ...
      config.java = {
        ...
        absolutePath: config.java?.absolutePath ? config.java.absolutePath : config.java?.relativePath ? node_path_1.default.join(utils_1.default.getServerFolder(config.serverId), config.java.relativePath, "/") : node_path_1.default.join(utils_1.default.getServerFolder(config.serverId), "runtime", "jre-${X}", "bin", "java"),
      ...
      this.config = { ...config, root: utils_1.default.getServerFolder(config.serverId) };
    }
    /**
     * Launch Minecraft.
     *
    ...
    async launch() {
      ...
      const java2 = new java_1.default(manifest.id, this.config.serverId);
      ...
      const javaFilesToDownload = await downloader2.getFilesToDownload(javaFiles.java);
      const modpackFilesToDownload = await downloader2.getFilesToDownload(modpackFiles.modpack);
      const librariesFilesToDownload = await downloader2.getFilesToDownload(librariesFiles.libraries);
      const assetsFilesToDownload = await downloader2.getFilesToDownload(assetsFiles.assets);
      const log4jFilesToDownload = await downloader2.getFilesToDownload(log4jFiles.log4j);
      ...
      await java2.check(this.config.java.absolutePath, manifest.javaVersion?.majorVersion ?? 8);
      ...
      await cleaner2.clean(files, this.config.cleaning.ignored, !this.config.cleaning.clean);
    ...
    async run(javaPath, args) {
      const minecraft = (0, node_child_process_1.spawn)(javaPath, args, { cwd: this.config.root, detached: true });
...
var hasRequiredEmlLib;
function requireEmlLib() {
  if (hasRequiredEmlLib) return emlLib;
  hasRequiredEmlLib = 1;
  var __importDefault = emlLib && emlLib.__importDefault || function(mod) {
    return mod && mod.__esModule ? mod : { "default": mod };
  };
  ...
  const microsoft_1 = __importDefault(requireMicrosoft());
  emlLib.MicrosoftAuth = microsoft_1.default;
  const azuriom_1 = __importDefault(requireAzuriom());
  emlLib.AzAuth = azuriom_1.default;
  const crack_1 = __importDefault(requireCrack());
  emlLib.CrackAuth = crack_1.default;
  const bootstraps_1 = __importDefault(requireBootstraps());
  emlLib.Bootstraps = bootstraps_1.default;
  const maintenance_1 = __importDefault(requireMaintenance());
  emlLib.Maintenance = maintenance_1.default;
  const news_1 = __importDefault(requireNews());
  emlLib.News = news_1.default;
  const background_1 = __importDefault(requireBackground());
  emlLib.Background = background_1.default;
  const serverstatus_1 = __importDefault(requireServerstatus());
  emlLib.ServerStatus = serverstatus_1.default;
  const java_1 = __importDefault(requireJava());
  emlLib.Java = java_1.default;
  const launcher_1 = __importDefault(requireLauncher());
  ...
  const EMLLib = {
     ...
     */
    MicrosoftAuth: microsoft_1.default,
    /**
     * Authenticate a user with [Azuriom](https://azuriom.com/).
     */
    AzAuth: azuriom_1.default,
    /**
     * Authenticate a user with a crack account.
     * @deprecated This auth method is not secure, use it only for testing purposes or for local servers!
     */
    CrackAuth: crack_1.default,
    /**
     ...
     * @workInProgress
     */
    Bootstraps: bootstraps_1.default,
    /**
     ...
     */
    ServerStatus: serverstatus_1.default,
    /**
  ...
  emlLib.default = EMLLib;
  return emlLib;
}
var emlLibExports = requireEmlLib();
const sessionPath = path__namespace.join(require$$0.app.getPath("userData"), "session.json");
function registerAuthHandlers(mainWindow2) {
  const auth = new emlLibExports.MicrosoftAuth(mainWindow2);
  require$$0.ipcMain.handle("auth:login", async () => {
    try {
      const account = await auth.auth();
      console.log(sessionPath);
      fs__namespace.writeFileSync(sessionPath, JSON.stringify(account));
      return { success: true, account };
    } catch (err) {
      return { success: false, error: err.message ?? "Unknown error" };
    }
  });
  require$$0.ipcMain.handle("auth:refresh", async () => {
  ...
  require$$0.ipcMain.handle("auth:logout", async () => {
    if (fs__namespace.existsSync(sessionPath)) {
...
function registerLauncherHandlers(mainWindow2) {
  ...
  require$$0.ipcMain.handle("game:launch", (_event, payload) => {
    ...
    launcher2.on("launch_compute_download", () => {
      console.log("\nComputing download...");
      mainWindow2.webContents.send("game:launch_compute_download");
    });
    launcher2.on("launch_download", (download) => {
      console.log(`
Downloading ${download.total.amount} files (${download.total.size} B).`);
      mainWindow2.webContents.send("game:launch_download", download);
    });
    launcher2.on("download_progress", (progress) => {
      console.log(progress.type, `=> Downloaded ${Math.round(progress.downloaded.size / 1024)} / ${Math.round(progress.total.size / 1024)} kB`);
      mainWindow2.webContents.send("game:download_progress", progress);
    });
    launcher2.on("download_error", (error) => {
      console.error(error.type, `=> Error downloading ${error.filename}: ${error.message}`);
      mainWindow2.webContents.send("game:download_error", error);
    });
    launcher2.on("download_end", (info) => {
      console.log(`Downloaded ${info.downloaded.amount} files.`);
      mainWindow2.webContents.send("game:download_end", info);
    });
    launcher2.on("launch_install_loader", (loader) => {
      console.log(`
Installing loader ${loader.type} ${loader.loaderVersion}...`);
      mainWindow2.webContents.send("game:launch_install_loader", loader);
    });
    launcher2.on("launch_extract_natives", () => {
      console.log("\nExtracting natives...");
      mainWindow2.webContents.send("game:launch_extract_natives");
    });
    launcher2.on("extract_progress", (progress) => {
      console.log(`Extracted ${progress.filename}.`);
      mainWindow2.webContents.send("game:extract_progress", progress);
    });
    launcher2.on("extract_end", (info) => {
      console.log(`Extracted ${info.amount} files.`);
      mainWindow2.webContents.send("game:extract_end", info);
    });
    launcher2.on("launch_copy_assets", () => {
      console.log("\nCopying assets...");
      mainWindow2.webContents.send("game:launch_copy_assets");
    });
    launcher2.on("copy_progress", (progress) => {
      console.log(`Copyed ${progress.filename} to ${progress.dest}.`);
      mainWindow2.webContents.send("game:copy_progress", progress);
    });
    launcher2.on("copy_end", (info) => {
      console.log(`Copied ${info.amount} files.`);
      mainWindow2.webContents.send("game:copy_end", info);
    });
    launcher2.on("launch_patch_loader", () => {
      console.log("\nPatching loader...");
      mainWindow2.webContents.send("game:launch_patch_loader");
    });
    launcher2.on("patch_progress", (progress) => {
      console.log(`Patched ${progress.filename}.`);
      mainWindow2.webContents.send("game:patch_progress", progress);
    });
    launcher2.on("patch_error", (error) => {
      console.error(`Error patching ${error.filename}: ${error.message}`);
      mainWindow2.webContents.send("game:patch_error", error);
    });
    launcher2.on("patch_end", (info) => {
      console.log(`Patched ${info.amount} files.`);
      mainWindow2.webContents.send("game:patch_end", info);
    });
    launcher2.on("launch_check_java", () => {
      console.log("\nChecking Java...");
      mainWindow2.webContents.send("game:launch_check_java");
    });
    launcher2.on("java_info", (info) => {
      console.log(`Using Java ${info.version} ${info.arch}`);
      mainWindow2.webContents.send("game:java_info", info);
    });
    launcher2.on("launch_clean", () => {
      console.log("\nCleaning game directory...");
      mainWindow2.webContents.send("game:launch_clean");
    });
    launcher2.on("clean_progress", (progress) => {
      console.log(`Cleaned ${progress.filename}.`);
      mainWindow2.webContents.send("game:clean_progress", progress);
    });
    launcher2.on("clean_end", (info) => {
      console.log(`Cleaned ${info.amount} files.`);
      mainWindow2.webContents.send("game:clean_end", info);
    });
    launcher2.on("launch_launch", (info) => {
      console.log(`
Launching Minecraft ${info.version} (${info.type}${info.loaderVersion ? ` ${info.loaderVersion}` : ""})...`);
    ...
    launcher2.on("launch_data", (message) => {
      console.log(message);
      mainWindow2.webContents.send("game:launch_data", message);
    });
    launcher2.on("launch_close", (code) => {
      console.log(`Closed with code ${code}.`);
      mainWindow2.webContents.send("game:launch_close", code);
    });
    launcher2.on("launch_debug", (message) => {
      console.log(`Debug: ${message}
`);
      mainWindow2.webContents.send("game:launch_debug", message);
    });
    launcher2.on("patch_debug", (message) => {
      console.log(`Debug: ${message}`);
      mainWindow2.webContents.send("game:patch_debug", message);
    });
    try {
      launcher2.launch();
    } catch (err) {
...
const settingsPath = path__namespace.join(require$$0.app.getPath("userData"), "settings.json");
...
function registerSettingsHandlers() {
  require$$0.ipcMain.handle("settings:get", async () => {
    try {
      if (!fs__namespace.existsSync(settingsPath)) {
        fs__namespace.writeFileSync(settingsPath, JSON.stringify(DEFAULT_SETTINGS, null, 2));
        return DEFAULT_SETTINGS;
      }
      const data = fs__namespace.readFileSync(settingsPath, "utf-8");
  ...
  require$$0.ipcMain.handle("settings:set", async (_event, newSettings) => {
    try {
      fs__namespace.writeFileSync(settingsPath, JSON.stringify(newSettings, null, 2));
      return true;
    } catch (err) {
  ...
  require$$0.ipcMain.handle("system:info", async () => {
    const totalMemBytes = require$$3__namespace.totalmem();
  ...
  require$$0.ipcMain.handle("settings:pick-java", async () => {
...
function registerServerHandlers() {
  require$$0.ipcMain.handle("server:status", async (_event, ip, port = 25565) => {
    try {
      const server = new emlLibExports.ServerStatus(ip, port, "modern", 774);
...
function configureAppMenu() {
  require$$0.app.setAboutPanelOptions({
    applicationName: APP_TITLE,
    applicationVersion: require$$0.app.getVersion(),
