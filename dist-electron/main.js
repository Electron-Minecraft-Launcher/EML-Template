...
import require$$6 from "node:fs/promises";
import require$$4$1 from "node:child_process";
  ...
  class MicrosoftAuth {
    ...
    async auth() {
      ...
      let res = await fetch("https://login.live.com/oauth20_token.srf", {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Error while getting the OAuth2 token");
      });
      try {
    ...
    async getAccount(authInfo) {
      let xboxLive = await fetch("https://user.auth.xboxlive.com/user/authenticate", {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Error while getting the Xbox Live token");
      });
      let xsts = await fetch("https://xsts.auth.xboxlive.com/xsts/authorize", {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Error while getting the XSTS token");
      });
      await fetch("https://api.minecraftservices.com/launcher/login", {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Error while launching the game");
      });
      let mcLogin = await fetch("https://api.minecraftservices.com/authentication/login_with_xbox", {
        ...
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Error while logging into Minecraft");
      });
    ...
    async getProfile(mcLogin) {
      ...
      if (profile.error)
        throw new errors_1.EMLLibError(errors_1.ErrorType.AUTH_ERROR, "Error while getting the Minecraft profile");
      return {
        uuid: profile.id,
        name: profile.name
      };
...
function requireUtils$1() {
  ...
  const errors_1 = requireErrors$1();
  const node_path_1 = __importDefault(require$$1$1);
  const node_fs_1 = __importDefault(require$$2$1);
  const node_crypto_1 = __importDefault(require$$0$1);
  ...
  class Utils {
    ...
    getFileHash(filePath) {
      try {
        const fileHash = node_fs_1.default.readFileSync(filePath);
        return node_crypto_1.default.createHash("sha1").update(fileHash).digest("hex");
      } catch (err) {
     ...
     */
    getLibraryPath(libName, ...path) {
      const l = libName.replace(/@([a-z]*)$/, "").split(":");
      return node_path_1.default.join(...path, `${l[0].replace(/\./g, "/")}/${l[1]}/${l[2]}/`);
    }
    /**
...
function requireDownloader() {
  ...
  class Downloader extends events_1.default {
    ...
    async download(files, skipCheck = false) {
      ...
      const max2 = Math.min(filesToDownload.length, 5);
    ...
    getFilesToDownload(files) {
      ...
      files.forEach((file) => {
        ...
        } else if ((!node_fs_1.default.existsSync(filePath) || file.sha1 !== utils_1.default.getFileHash(filePath)) && file.url) {
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          res.body.on("error", (err) => {
            ...
            this.error = true;
            reject(new errors_1.EMLLibError(errors_1.ErrorType.DOWNLOAD_ERROR, `Error while downloading ${file.name}: ${err}`));
          });
    ...
    async chmodJavaFiles(filePath, file) {
      if (process.platform !== "win32") {
        if (file.executable) {
          try {
            await (0, promises_1.chmod)(filePath, 493);
          } catch (error) {
            this.emit("download_error", {
              filename: file.name,
              type: file.type,
...
function requireBootstraps() {
  ...
  const events_1 = __importDefault(requireEvents());
  const node_path_1 = __importDefault(require$$1$1);
  const node_child_process_1 = require$$4$1;
  const errors_1 = requireErrors$1();
    ...
    async getStatus() {
      return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          throw new errors_1.EMLLibError(errors_1.ErrorType.NET_ERROR, "Connection timed out");
        }, this.timeout * 1e3);
        socket.on("connect", () => {
...
function requireJava() {
  ...
  class Java extends events_1.default {
     ...
     */
    check(absolutePath = node_path_1.default.join(utils_1.default.getServerFolder(this.serverId), "runtime", "jre-${X}", "bin", "java"), majorVersion = 8) {
      const check = (0, node_child_process_1.spawnSync)(`"${absolutePath.replace("${X}", majorVersion + "")}"`, ["-version"], { shell: true });
      if (check.error)
        throw new errors_1.EMLLibError(errors_1.ErrorType.JAVA_ERROR, `Java is not correctly installed: ${check.error.message}`);
      const res = {
        version: check.output.map((o) => o?.toString("utf8")).join(" ").match(/"(.*?)"/)?.pop() ?? majorVersion + "",
        arch: check.output.map((o) => o?.toString("utf8")).join(" ").includes("64-Bit") ? "64-bit" : "32-bit"
      };
...
function requireFilesmanager() {
  ...
  class FilesManager extends events_1.default {
    ...
    async getLibraries() {
      let files = [];
      let libraries = [];
      if (!node_fs_1.default.existsSync(node_path_1.default.join(this.config.root, "versions", this.manifest.id))) {
        node_fs_1.default.mkdirSync(node_path_1.default.join(this.config.root, "versions", this.manifest.id), { recursive: true });
      }
      files.push({ name: `${this.manifest.id}.json`, path: node_path_1.default.join("versions", this.manifest.id, "/"), url: "", type: "OTHER" });
      node_fs_1.default.writeFileSync(node_path_1.default.join(this.config.root, "versions", this.manifest.id, `${this.manifest.id}.json`), JSON.stringify(this.manifest, null, 2));
      this.manifest.libraries.forEach((lib2) => {
        ...
        if (artifact) {
          if (artifact.path) {
            ...
            path = node_path_1.default.join("libraries", node_path_1.default.dirname(artifact.path), "/");
          } else {
            name = utils_1.default.getLibraryName(lib2.name);
            path = utils_1.default.getLibraryPath(lib2.name, "libraries");
          }
          libraries.push({
            name,
            path,
    ...
    async getAssets() {
      ...
      if (!node_fs_1.default.existsSync(node_path_1.default.join(this.config.root, "assets", "indexes"))) {
        node_fs_1.default.mkdirSync(node_path_1.default.join(this.config.root, "assets", "indexes"), { recursive: true });
      }
      files.push({ name: `${this.manifest.assets}.json`, path: node_path_1.default.join("assets", "indexes", "/"), url: "", type: "OTHER" });
      node_fs_1.default.writeFileSync(node_path_1.default.join(this.config.root, "assets", "indexes", `${this.manifest.assets}.json`), JSON.stringify(res, null, 2));
    ...
    extractNatives(libraries) {
      ...
      const nativesFolder = node_path_1.default.join(this.config.root, "bin", "natives");
      let files = [];
      if (!node_fs_1.default.existsSync(nativesFolder)) {
        node_fs_1.default.mkdirSync(nativesFolder, { recursive: true });
      }
      natives.forEach((native2) => {
        if (!node_fs_1.default.existsSync(node_path_1.default.join(this.config.root, native2.path, native2.name)))
          return;
        ...
        zip.getEntries().forEach((entry) => {
          if (!entry.entryName.startsWith("META-INF")) {
            if (entry.isDirectory) {
              node_fs_1.default.mkdirSync(node_path_1.default.join(nativesFolder, entry.entryName), { recursive: true });
            } else {
              node_fs_1.default.writeFileSync(node_path_1.default.join(nativesFolder, entry.entryName), zip.readFile(entry));
            }
            files.push({
              name: node_path_1.default.basename(entry.entryName),
              path: node_path_1.default.join("bin", "natives", node_path_1.default.dirname(entry.entryName), "/"),
              url: "",
              sha1: "",
    ...
    copyAssets() {
      ...
      if (this.manifest.assets === "legacy" || this.manifest.assets === "pre-1.6") {
        if (node_fs_1.default.existsSync(node_path_1.default.join(this.config.root, "assets", "legacy"))) {
        ...
        const assets = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(this.config.root, "assets", "indexes", `${this.manifest.assets}.json`), "utf-8"));
        Object.entries(assets.objects).forEach(([path, { hash, size }]) => {
          const assetLegacyPath = node_path_1.default.join("resources", node_path_1.default.dirname(path));
          const assetLegacyName = node_path_1.default.basename(path);
          if (!node_fs_1.default.existsSync(node_path_1.default.join(this.config.root, assetLegacyPath))) {
            node_fs_1.default.mkdirSync(node_path_1.default.join(this.config.root, assetLegacyPath), { recursive: true });
          }
          if (!node_fs_1.default.existsSync(node_path_1.default.join(assetLegacyPath, assetLegacyName))) {
            node_fs_1.default.copyFileSync(node_path_1.default.join(this.config.root, "assets", "objects", hash.substring(0, 2), hash), node_path_1.default.join(this.config.root, assetLegacyPath, assetLegacyName));
          }
          files.push({
  ...
  class Cleaner extends events_1.default {
    ...
    clean(files, ignore = [], skipClean = false) {
      ...
      this.browsed.forEach((file) => {
        const fullPath = node_path_1.default.join(file.path, file.name);
        if (!files.find((f) => node_path_1.default.join(this.dest, f.path, f.name) === fullPath) && !ignore.find((ig) => fullPath.startsWith(node_path_1.default.join(this.dest, ig)))) {
    ...
    browse(dir) {
      ...
      files.forEach((file) => {
        if (node_fs_1.default.statSync(node_path_1.default.join(dir, file)).isDirectory()) {
          this.browse(node_path_1.default.join(dir, file));
        } else {
          this.browsed.push({
            name: file,
...
function requireForgeloader() {
  ...
  class ForgeLoader extends events_1.default {
    ...
    async setup() {
      ...
      if (!node_fs_1.default.existsSync(forgePath))
        node_fs_1.default.mkdirSync(forgePath, { recursive: true });
    ...
    extractZip(forgePath, minecraftPath, zip, jar) {
      ...
      jar.writeZip(node_path_1.default.join(minecraftPath, `${this.manifest.id}.jar`));
      ...
      node_fs_1.default.writeFileSync(node_path_1.default.join(forgePath, `${forgeManifest.id}.json`), JSON.stringify(forgeManifest, null, 2));
    ...
    async extractJar(forgePath, zip) {
      ...
      if (installProfile.install) {
        ...
        forgeManifest = JSON.parse(zip.getEntry(node_path_1.default.basename(installProfile.json))?.getData().toString("utf8") + "");
      }
      node_fs_1.default.writeFileSync(node_path_1.default.join(forgePath, `forge-${this.loader.loaderVersion}.json`), JSON.stringify(forgeManifest, null, 2));
      ...
      if (installProfile.filePath) {
        ...
        if (!node_fs_1.default.existsSync(universalExtractPath))
          node_fs_1.default.mkdirSync(universalExtractPath, { recursive: true });
        node_fs_1.default.writeFileSync(node_path_1.default.join(universalExtractPath, universalName), zip.getEntry(installProfile.filePath).getData());
        ...
        if (!node_fs_1.default.existsSync(universalExtractPath))
          node_fs_1.default.mkdirSync(universalExtractPath, { recursive: true });
        zip.getEntries().filter((entry) => node_path_1.default.join(entry.entryName).includes(node_path_1.default.join("maven", universalPath))).forEach((entry) => {
          if (!entry.entryName.endsWith(".jar"))
            return;
          node_fs_1.default.writeFileSync(node_path_1.default.join(universalExtractPath, node_path_1.default.basename(entry.entryName)), entry.getData());
          libraries.push({
            name: node_path_1.default.basename(entry.entryName),
      ...
      if (installProfile.processors && installProfile.processors.length > 0) {
        ...
        if (!node_fs_1.default.existsSync(clientDataExtractPath))
          node_fs_1.default.mkdirSync(clientDataExtractPath, { recursive: true });
        node_fs_1.default.writeFileSync(node_path_1.default.join(clientDataExtractPath, clientDataName), clientData);
      ...
      for (const lib2 of libs) {
        ...
        if (artifact) {
          if (artifact.path) {
            ...
            path = node_path_1.default.join("libraries", artifact.path.split("/").slice(0, -1).join("/"), "/");
          } else {
            name = utils_1.default.getLibraryName(lib2.name);
            if (type === "NATIVE")
  ...
  class Patcher extends events_1.default {
    ...
    isPatched() {
      ...
      for (let lib2 of libraries) {
        ...
        files.push({ name: libName, path: node_path_1.default.join("libraries", libPath), url: "", type: "LIBRARY" });
        if (!node_fs_1.default.existsSync(node_path_1.default.join(libExtractPath, libName)))
          patched = false;
      }
      return { patched, files };
    }
    getJarMain(jarPath) {
      if (!node_fs_1.default.existsSync(jarPath))
        return null;
    ...
    getClasspath(libraries) {
      ...
      for (let i = 0; i < libraries.length; i++) {
        ...
        if (lib2.type === "LIBRARY") {
          const path = node_path_1.default.join(this.config.root, lib2.path, lib2.name).replaceAll("\\", "/");
...
function requireLauncher() {
  ...
  class Launcher extends events_1.default {
    ...
    async launch() {
      ...
      java2.check(this.config.java.absolutePath, manifest.javaVersion?.majorVersion || 8);
...
function requireEmlLib() {
  ...
  emlLib.default = EMLLib2;
  return emlLib;
}
var emlLibExports = requireEmlLib();
const EMLLib = /* @__PURE__ */ getDefaultExportFromCjs(emlLibExports);
...
app.whenReady().then(async () => {
  ...
  const launcher2 = new EMLLib.Launcher({
    url: "http://localhost:8080",
    // Your AdminTool URL
    serverId: "template",
    // Your server ID
    account: new EMLLib.CrackAuth().auth("TestAccount")
    // A test cracked account
  });
  launcher2.on("launch_compute_download", () => console.log("Computing download..."));
  launcher2.on("launch_download", (download) => console.log(`Downloading ${download.total.amount} files (${download.total.size} B).`));
  launcher2.on("launch_install_loader", (loader) => console.log(`Installing loader ${loader.type} ${loader.loaderVersion}...`));
  launcher2.on("launch_extract_natives", () => console.log("Extracting natives..."));
  launcher2.on("launch_copy_assets", () => console.log("Copying assets..."));
  launcher2.on("launch_patch_loader", () => console.log("Patching loader..."));
  launcher2.on("launch_check_java", () => console.log("Checking Java..."));
  launcher2.on("java_info", (info) => console.log(`Using Java ${info.version} ${info.arch}`));
  launcher2.on("launch_clean", () => console.log("\nCleaning game directory..."));
  launcher2.on("launch_launch", (info) => console.log(`Launching Minecraft ${info.version}...`));
  launcher2.on("launch_data", (message) => console.log(message));
  launcher2.on("launch_close", (code) => console.log(`Closed with code ${code}.`));
  try {
